...
class StateManager {
  ...
  async setUserProfile() {
    try {
      ...
      // 2. If not found, fallback to MongoDB
      if (!this.userProfile) {
        const User = require('../../db/models/user'); // Dynamic import
        const mongoProfile = await User.findById(this.user);

        if (mongoProfile) {
          this.userProfile = mongoProfile.toObject();
        } else {
          throw new Error('User profile not found in Redis or MongoDB');
        }
      }
